...
public class AuthenticationUtils implements CallbackHandler
{
    ...
    private static final String WS_SECURITY_INFO = 
         ...
         "               <parameter name='passwordCallbackClass' value='com.arte.gestordocumental.util.concurrencia.AuthenticationUtils'/>" +
         "               <parameter name='passwordType' value='PasswordText'/>" +
         "           </handler>" +
         "       <handler name='cookieHandler' type='java:com.arte.gestordocumental.util.concurrencia.CookieHandler' />" +
         "     </requestFlow >" +
         "   </globalConfiguration>" +
         "</deployment>";
     ...
     */
    public static void startSession(WebServiceFactoryConfig webServiceFactoryConfig)
        throws AuthenticationFault
    {
        try
        {
            // Start the session
            AuthenticationResult result = WebServiceFactory.getAuthenticationService(webServiceFactoryConfig).startSession(webServiceFactoryConfig.getUser(), webServiceFactoryConfig.getPassword());           
     ...
     */
    public static void endSession(WebServiceFactoryConfig webServiceFactoryConfig)
    {
        ...
        if (authenticationDetails != null)
        {
            try
            {
                WebServiceFactory.getAuthenticationService(webServiceFactoryConfig).endSession(authenticationDetails.getTicket());
