...
SwaggerModel = (function() {
  ...
  SwaggerModel.prototype.getMockSignature_old = function(prefix, modelToIgnore) {
    ...
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      ...
      if ((prop.refModel != null) && (!(prop.refModel === modelToIgnore))) {
        returnVal = returnVal + ('<br>' + prop.refModel.getMockSignature_old(void 0, this));
      }
    }
    return returnVal;
  };
  ...
  SwaggerModel.prototype.getMockSignature = function(prefix, modelToIgnore) {
    var ignore, prop, properties, propertiesStr, returnVal, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
    ignore = modelToIgnore || {};
    if (ignore[this.name]) {
      return '';
    }
    ignore[this.name] = this.name;
    propertiesStr = [];
    _ref = this.properties;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      prop = _ref[_i];
    ...
    properties = "<ul class='swagger-signature-class-properties'>";
    _ref1 = this.properties;
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      prop = _ref1[_j];
      properties += "<li>" + (prop.toString()) + "</li>";
    }
    properties += "</ul>";
    returnVal = "<div class=\"swagger-signature-class\">\n  <div class=\"swagger-signature-class-name\">\n      <a id=\"" + this.name + "\">" + this.name + "</a>\n  </div>\n  <div class=\"swagger-signature-class-properties\">\n      " + properties + "\n  </div>\n</div>";
    _ref2 = this.properties;
    for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
      prop = _ref2[_k];
      if ((prop.refModel != null) && (!(prop.refModel === modelToIgnore))) {
        returnVal = returnVal + prop.refModel.getMockSignature(void 0, ignore);
      }
    }
    return returnVal;
  };
...
SwaggerModelProperty = (function() {
  ...
  SwaggerModelProperty.prototype.isBasicType = function(type) {
    var basicTypes;
    basicTypes = ["byte", "boolean", "int", "long", "float", "double", "string", "date"];
    return basicTypes.indexOf(type.toLowerCase()) !== -1;
  };

  SwaggerModelProperty.prototype.typeWithLink = function(type) {
    if (this.isBasicType(type)) {
      return type;
    } else {
      return "<a class=\"swagger-signature-type\" href=\"#" + type + "\">" + type + "</a>";
    }
  };
  ...
  SwaggerModelProperty.prototype.toString = function() {
    ...
    var str;
    if (this.isArray) {
      str = "<span class=\"swagger-signature-prop-name\">" + this.name + "</span> : <span class=\"swagger-signature-prop-type\">" + this.dataType + "[" + (this.typeWithLink(this.refDataType)) + "]</span>";
    } else {
      str = "<span class=\"swagger-signature-prop-name\">" + this.name + "</span> : <span class=\"swagger-signature-prop-type\">" + (this.typeWithLink(this.dataType)) + "</type>";
    }
    if (this.values != null) {
      str += " = ['" + this.values.join("' or '") + "']";
    }
    if (this.descr != null) {
      str += " <div class='swagger-signature-prop-description'>" + this.descr + "</div>";
    }
    return str;
  };
