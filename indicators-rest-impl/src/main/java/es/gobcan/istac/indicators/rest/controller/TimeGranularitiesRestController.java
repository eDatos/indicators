...
public class TimeGranularitiesRestController extends AbstractRestController {
    ...
    @ResponseBody
    public ResponseEntity<ListResultType<MetadataGranularityType>> findTimeGranularities(final UriComponentsBuilder uriComponentsBuilder) throws MetamacException {
        List<MetadataGranularityType> items = timeRestFacade.findTimeGranularities();

        String selfLink = UriUtils.getTimeGranularitiesLink(uriComponentsBuilder);
        ListResultType<MetadataGranularityType> itemsResultType = new ListResultType<MetadataGranularityType>(RestConstants.KIND_TIME_GRANULARITIES, selfLink, items);
